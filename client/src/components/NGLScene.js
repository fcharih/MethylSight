...
class NGLScene extends Component {
  ...
  draw(props) {
    this.stage.loadFile(`https://files.rcsb.org/download/${props.pdbId}.pdb`).then((component) => {
  ...
  render() {
    return (
      <div className="ngl-scene-wrapper">
        <div className="ngl-scene" style={{ height: this.props.height, width: this.props.width, border: "3px solid black" }} ref={node => this.nglScene = node} />
      </div>
    );
  }
}
